...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import libpcap_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("libpcap_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "libpcap_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"libpcap", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "libpcap_jll", "src", "libpcap_jll.jl"))
        @info("libpcap_ll dev'ed out to /home/mateusz/.julia/dev/libpcap_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("f78539f5-9ba6-5ded-ae79-6d41194c59b7"))
platforms = [Pkg.Artifacts.unpack_platform(e, "libpcap", artifacts_toml) for e in artifacts["libpcap"]]
...
if best_platform === nothing
    @debug("Unable to load libpcap; unsupported platform $(triplet(platform_key_abi()))")
else
